...
if (isset($_POST["submit"])) { // if submit button was clicked
    ...
    $fileSize = $file["size"];
    $fileError = $file["error"]; // if an error occured
    // $fileType = $file["type"]; // get the file type

    // explode(delimiter, fileName) = take appart the name (returns array)
    $fileExt = explode(".", $fileName);
    // end gets last element in array
    $fileRealExt = strtolower(end($fileExt));
    $allowedExtentions = array("jpg", "jpeg", "png");
    print_r($_FILES["upload"]);
    if (in_array($fileRealExt, $allowedExtentions)) { // check if the file ext is in array
        if ($fileError === 0) { // if no error occured
